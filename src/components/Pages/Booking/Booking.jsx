...
const Booking = () => {
  ...
  const bookingData = [
    {
      image: "/Images/ssi2.png",
      title: "Book Your Free Consultation",
      subtitle: "Schedule a consultation with one of our expert visa advisors.",
    },
  ];

  return (
    <div className="py-10 flex flex-col gap-6 bg-[#F9FAFB] rounded-2xl">
      <h1 className="text-2xl font-semibold w-11/12 mx-auto ">Booking</h1>
      ...
      {previewIndex !== null && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white flex flex-col gap-4  p-6 rounded-lg w-[500px] transform transition-all duration-500 ease-out scale-90 opacity-0 animate-popup">
            <img
              src={bookingData[previewIndex].image}
              alt={bookingData[previewIndex].title}
              className="w-[250px]  rounded  object-cover"
            />

            <h2 className="text-xl font-semibold ">
              {bookingData[previewIndex].title}
            </h2>
            <h2 className=" ">
              {bookingData[previewIndex].subtitle}
            </h2>
